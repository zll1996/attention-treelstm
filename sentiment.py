...
def main():
    ...
    model = TreeLSTMSentiment(
                args.cuda, vocab.size(), args.input_dim, args.mem_dim, args.num_classes, args.model_name,
                args.attention_dim, args.dropout2, args.attention_flag, criterion)
    ...
    if mode == 'DEBUG':
        ...
        embedding_model = torch.load(args.saved + str(epoch) + '_embedding_' + filename)
        trainer = SentimentTrainer(args, model, embedding_model, criterion, optimizer)
        test_loss, test_pred = trainer.test(test_dataset)
        test_acc = metrics.sentiment_accuracy_score(test_pred, test_dataset.labels)
